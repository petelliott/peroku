  ...
  (config:with-config (#P".peroku.json")
    (cond
      ((string= (car args) "list")
       (core:list-projects config:*token* config:*peroku*))
      ((string= (car args) "up")
       (core:up config:*token* config:*peroku* config:*project* config:*rule*))
      ((string= (car args) "down")
       (core:down config:*token* config:*peroku* config:*project*))
